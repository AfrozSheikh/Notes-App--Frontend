...
export const Home = () => {
  ...
  const deleteNode = async (id) => {
    try {
      const response = await axios.delete(`https://notes-app-backend-gdcr.onrender.com/api/note/${id}`, {
  ...
  const editNote = async (id, title, description) => {
    try {
      const response = await axios.put(`https://notes-app-backend-gdcr.onrender.com/api/note/${id}`, { title, description }, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      if (response.data.success) {
        fetchNotes();
  ...
  const addNote = async (title, description) => {
    try {
      const response = await axios.post("https://notes-app-backend-gdcr.onrender.com/api/note/add", { title, description }, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      if (response.data.success) {
        fetchNotes();
  ...
  const fetchNotes = async () => {
    try {
      const { data } = await axios.get("https://notes-app-backend-gdcr.onrender.com/api/note", {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      setNotes(data.notes);
